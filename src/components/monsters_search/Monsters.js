        ...
        <div className="monster__img" id="second__screen">
            <div className="container">
                <div className="monsters">
                    <form>
                        <input type="text" className="mob__search" placeholder="Search" onChange={(e) => setSearchTerm(e.target.value)} />
                    </form>
                    <div className="mob__found" id="mob__found__list">
                        {mobsList.slice(0, 6).map((mob, index) => { if (searchTerm !== '') { return <dt key={index} id={mob.url} onClick={(e) => setShowMob(e.target.id)} className="search__res">{mob.name}</dt> } else return '' })}
                    </div>
                    <div className="monster__info">
                        <img src={monster.image !== undefined ? apiUrl + monster.image : require('../../img/unnamed.png')} alt="use ur imagination" className="monster__pic" id="mob__pic" />
